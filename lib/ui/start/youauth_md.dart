...
import 'package:youphotomobile/util/md_history.dart';
...
class _MdLoginPageState extends State<MdLoginPage> {
  ...
  Future<void> _fetchServices() async {
    ...
    if (resp.data is Map && resp.data['data'] is List) {
      ...
      _services = list
          ...
          .where((e) => (e.name.toString().toLowerCase()).contains('photo'))
          .toList()
          .cast<MdServiceItem>();
      // 如果只有一个服务，自动选择
  ...
  Future<void> _selectService(MdServiceItem item) async {
    ...
    await ApplicationConfig().saveServiceUrl(baseUrl);
    await ApplicationConfig().saveToken(_mdToken ?? "");
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Padding(
                        ...
                        child: ActionChip(
                          ...
                          onPressed: () async {
                            await ApplicationConfig().saveServiceUrl(
