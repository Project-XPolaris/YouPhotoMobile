...
class TabHomeVerticalPage extends StatelessWidget {
  ...
  const TabHomeVerticalPage({Key? key})
      : super(key: key);
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<TabHomeBloc, TabHomeState>(
      builder: (context, state) {
        ...
        var controller = createLoadMoreController(() => context.read<TabHomeBloc>().add(LoadMoreEvent()));
        return RefreshIndicator(
          color: Theme
              ...
              .primary,
          onRefresh: () async {
            context.read<TabHomeBloc>().add(UpdateFilterEvent(filter: state.filter));
          },
          child: LayoutBuilder(
              ...
              return GridView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return GestureDetector(
                    onTap: () async {
                      await ImageViewer.Launch(context, context.read<TabHomeBloc>().loader, index,(changedIndex) {
                        ...
                        int itemInRowIndex = ((changedIndex.toDouble() + 1) / crossAxisCount).ceil();
                        // calc offset in center of grid
                        double offset = (itemHeight * itemInRowIndex) - (crossAxisSize / 2);
